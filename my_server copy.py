...
@mcp.tool(
    ...
    description="对信息搜集表进行分析，生成转录组分析结果。", # 自定义描述
...
@mcp.tool(
    name="dna_seq_analysis",           # LLM 的自定义工具名称
    description="对信息搜集表进行分析，生成基因组分析结果。", # 自定义描述
    tags={"dna", "analysis"},      # 用于组织/过滤的可选标签
)
def dna_seq_analysis(
    xls_file: Annotated[str, "信息搜集表的路径"]
) -> str:
    """对信息搜集表进行分析。"""
